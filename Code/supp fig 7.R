...
#steps to define the working directory are outlined in the main text figure code files
    setwd("~/size-spectra-life-history/Model_output/suppfig7/")
...
truncM <- truncate_age(M, lifespan) 
 ...
 linetypes <-1
  library(wesanderson)
 pal<- wes_palette(6, name = "Darjeeling2", type = "continuous")
dev.new(height = 3.5,width = 4,units ="in")
 
 #pal = c("#C18CAF", "#2A5783")

  matplot(truncL, col=pal, type="l", lty=linetypes, xaxt="n", xlab = "", las=1, lwd=4,   ylab= "Length (cm)")
         axis(1, at = seq(0, (17)*12, by=12), labels = (seq(1, 18, by=1)))   
           legend(x=132, y=161, bty="n", col=pal,   lwd=3, cex=0.5, legend=c("K= 0.5, Phi = 0.1", "K= 0.5, Phi = 0.2", "K= 0.5, Phi = 0.3",  "K= 2, Phi = 0.1", "K= 2, Phi = 0.2","K= 2, Phi = 0.3"))
          
 plot(1:215, truncR[1:215, 6], type = "n", xaxt="n", las=1, xlab="", ylab = "Reproductive output (kg)")

 for (i in 1:length(index)){
   
   lines(lowess(1:(lifespan[index[i]]-1), truncR[1:(lifespan[index[i]]-1), index[i]], f = .5), lwd=3, lty=linetypes, col=pal[i])
