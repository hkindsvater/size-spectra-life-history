...
L<-c(100, 250)
      ...
      matplot(ProportionMaxR, t(Ecrit_survival), type="l", lwd=2, lty=1, xlim=c(0, 1.6e+08),  xlab = "60% of Lipid Stores (J)", ylab="Probability of survival")
       legend("bottomright", legend = c("Length = 100 cm", "Length = 250 cm"), cex = 0.8, lwd=2, col=c(1:2), bty="n" )
    # 
      #quartz()
    # par(mfrow=c(2, 2))
       f=c(0.5, 1, 2)
        ...
        Rlimit_smooth[i,] <-  (1 * Estores)  / (1 + f[i]*((1*Estores) / Rlimit)) 
         ...
         matplot(Estores/Jdensity, t(Rlimit_smooth), type="l", lwd=2, lty=1, col=c(2, 1, 4), xlim=c(0, 200), xlab = "Lipid Stores (kg)",   ylab="Fitness from allocation in this time increment")
