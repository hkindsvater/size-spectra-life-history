...
kappas <- c(1, 4, 8)
kap_inc <- matrix(nrow = 3, ncol = length(Mass))

for (k in 1:length(kappas)) {
  
 kap_inc[k, ] <- kappas[k]*3*Mass^(0.05)
}
 
matplot( Mass, t(kap_inc), type = "l", lty=1, lwd=2,   xlab="Mass (kg)", ylab = "Income (kg/month)", col = c("light green", "green", "dark green"))
...
for (h in 1:length(hseq) ) {
  ...
  matplot( Mass, t(exp(-mu_h)), type = "l", lty=1, lwd=2, ylim = c(0,1), xlab="Mass (kg)", ylab = "Per-month survival", col = c("light blue", "blue", "dark blue"))
  #ylab="Metabolic rate in J/season", col=c(4, 3, "orange", 2, "dark red")))
  ...
  matplot(Mass, t((temp_mat)/Jdensity), type = "l", lty=1, lwd=2,  xlab="Mass (kg)", ylab="Metabolic requirements in J/month", col=c("orange", "red", "dark red", "#301934") )  
