...
food_tab1 <- substr(length_filenames, 37, 41)
...
setwd("~/size-spectra-life-history/Model_output/suppfig8")
...
data295$coef1 <- c(rep(1e+16, 4), rep(1e+17, 4), rep(5e+15, 4), rep(5e+16, 4))

data295$kappa <- c(0.25, 0.5, 1, 2, 0.25, 0.5,1, 2,0.25, 0.5, 1, 2, 0.25, 0.5,1, 2)
data295$lifetime_R <- as.numeric(data295$lifetime_R)
data295$Max_R <- as.numeric(data295$Max_R)
 require(wesanderson)
 pal<- wes_palette(4, name = "Zissou1")
 dev.new(height = 4, width = 4, unit = "in")
ggplot(data = data295,  aes(x = kappa, y = Max_size, group = as.factor(coef1))) +
  geom_point(aes(color = as.factor(coef1)), shape = 19, size = 4) +       
  
  scale_color_manual(values = alpha(pal, 0.85), name = "Normalization coefficient" ) + 
...
ggplot(data = data295,  aes(x = kappa, y = lifetime_R, group = as.factor(coef1))) +
  geom_point(aes(color = as.factor(coef1)), shape = 19, size = 4) +       
